...
Types of arrays:
- static arrays (pointer-based access under the hood) and dynamic (class string, vector)
- one-dimensional and multi-dimensional (2d or more)
- homogeneous (arrays in C++), heterogeneous (lists in Python)

Syntax
Declaring an array:
dataType arrayName[arraySize];
where arraySize is the number of elements

Accessing elements:
arrayName[elementIndex];
where elementIndex must always be within the range [0; arraySize-1]

Array indexing is 0-based, so the first element always has index 0
*/
